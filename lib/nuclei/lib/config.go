...
func WithTemplateFilters(filters TemplateFilters) NucleiSDKOptions {
	return func(e *NucleiEngine) error {
		...
		pt := templateTypes.ProtocolTypes{}
		if err := pt.Set(filters.ProtocolTypes); err != nil {
			return err
		}
		ept := templateTypes.ProtocolTypes{}
...
// WithOptions applies application-owned configuration before the SDK
// initializes shared protocol state and clients.
func WithOptions(configure func(options *nucleiTypes.Options)) NucleiSDKOptions {
	return func(e *NucleiEngine) error {
		if configure != nil {
			configure(e.opts)
		}
...
// DisableDefaultIgnore keeps SDK initialization from adding the user-level
// nuclei ignore file to the application's explicit template filters.
func DisableDefaultIgnore() NucleiSDKOptions {
	return func(e *NucleiEngine) error {
		e.skipDefaultIgnore = true
		return nil
	}
}
